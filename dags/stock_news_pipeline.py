...
def stock_news_pipeline():  # Main DAG function
    ...
    def pull_dvc_history() -> str:  # Pull historical data from DVC
        ...
        try:
            # Run DVC commands
            subprocess.run(commands, cwd=tmp_dir, env=DVC_ENV, check=True, shell=True, executable="/bin/bash")
    ...
    def transform_and_profile(payload_json: str, history_json: str, **kwargs) -> str:
        ...
        report_period = f"{str_date.replace('T', ' ').replace('Z', '')} to {end_date.replace('T', ' ').replace('Z', '')}"
        ...
        # Deduplicate based on title and published date
        df_combined = df_combined.drop_duplicates(subset=['title', 'dedupe_date'], keep='last')
        ...
        if len(df_combined) <= history_len and history_len > 0:  # No new unique data
            ...
            # Skip this task and downstream tasks
            raise AirflowSkipException("Data is identical to history. Skipping write and push.")
    ...
    def git_commit_and_push(dvc_content: str, **kwargs):  # Commit and push changes to GitHub
        ...
        if os.path.exists(tmp_dir):  # Clean up existing temp dir
    ...
    def train_model(dvc_content: str, **kwargs):  # Train and log model with MLflow
        ...
        features = ['hour_of_day', 'day_of_week', 'title_sentiment', 'desc_sentiment', 'content_sentiment']
